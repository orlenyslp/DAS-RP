...
from discovery.gateway_conditions.replayer import BPMNGraph, Trace, parse_dataframe
...
def preprocess_event_log(event_log, log_ids, sampling_size=DEFAULT_SAMPLING_SIZE):
    ...
    avoid_columns = [getattr(log_ids, field.name) for field in fields(log_ids)]
    for col in log_by_case.columns:
        if log_by_case[col].dtype == 'object':
            avoid_columns.append(col)
    avoid_columns = list(set(avoid_columns))

    log_by_case = scale_data(log_by_case, avoid_columns)
    return log_by_case
...
def sample_event_log_by_case(event_log: pd.DataFrame, log_ids: EventLogIDs, sampling_size: int = DEFAULT_SAMPLING_SIZE):
    ...
    if len(unique_cases) <= sampling_size:
        return event_log_sorted.drop(columns=['index'])
...
def scale_data(log, avoid_columns, threshold=1e+37):
    ...
    for col in log.columns:
        if col not in avoid_columns and pd.api.types.is_numeric_dtype(log[col]):
